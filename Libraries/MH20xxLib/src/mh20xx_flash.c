...
@verbatim   
 ...
 ===============================================================================
    ...
    [..] The FLASH_Option Bytes Programming_functions, includes the following functions:
       ...
       (+) FLASH_Status FLASH_OB_UserConfig(uint8_t OB_IWDG, uint8_t OB_MHOP, uint8_t OB_MHDBY);
   ...
   (#) Call one or several functions to program the desired option bytes 
      ...
      (++) FLASH_Status FLASH_OB_UserConfig(uint8_t OB_IWDG, uint8_t OB_MHOP, uint8_t OB_MHDBY) 
           => to configure the user option Bytes: IWDG, MHOP and the Standby.
      (++) FLASH_Status FLASH_OB_BOOTConfig(uint8_t OB_BOOT1)
...
/**
  * @brief  Programs the FLASH User Option Byte: IWDG_SW / RMH_MHOP / RMH_MHDBY.
  * @note   To correctly run this function, the FLASH_OB_Unlock() function must be called before.
  * @note   Call the FLASH_OB_Lock() to disable the flash control register access and the option
  *         bytes (recommended to protect the FLASH memory against possible unwanted operation)
  ...
  * @param  OB_MHOP: Reset event when entering MHOP mode.
  *          This parameter can be one of the following values:
  *             @arg OB_MHOP_NoRMH: No reset generated when entering in MHOP
  *             @arg OB_MHOP_RMH: Reset generated when entering in MHOP
  * @param  OB_MHDBY: Reset event when entering Standby mode.
  *          This parameter can be one of the following values:
  *             @arg OB_MHDBY_NoRMH: No reset generated when entering in MHANDBY
  *             @arg OB_MHDBY_RMH: Reset generated when entering in MHANDBY
  * @retval FLASH Status: The returned value can be: 
  *         FLASH_ERROR_PROGRAM, FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
  */
FLASH_Status FLASH_OB_UserConfig(uint8_t OB_IWDG, uint8_t OB_MHOP, uint8_t OB_MHDBY)
{
  ...
  assert_param(IS_OB_MHOP_SOURCE(OB_MHOP));
  assert_param(IS_OB_MHDBY_SOURCE(OB_MHDBY));
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    OB->USER = (uint16_t)((uint16_t)(OB_IWDG | OB_MHOP) | (uint16_t)(OB_MHDBY | 0xF8));
...
/**
  ...
  *             @arg OB_MHOP_NoRMH / OB_MHOP_RMH: No reset / Reset generated when entering in MHOP
  *             @arg OB_MHDBY_NoRMH / OB_MHDBY_RMH: No reset / Reset generated when entering in MHANDBY
