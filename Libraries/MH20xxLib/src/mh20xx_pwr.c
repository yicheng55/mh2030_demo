...
@verbatim
  ...
  *** Stop mode *** 
  ...
  [..] In Stop mode, all clocks in the VCORE domain are stopped, the PLL, the HSI,
    ...
    (+) Entry:
        (++) The Stop mode is entered using the PWR_EnterMHOPMode(PWR_Regulator_LowPower,) 
  ...
  *** Standby mode *** 
  ...
  [..] The voltage regulator is OFF.
    ...
    (+) Entry:
        (++) The Standby mode is entered using the PWR_EnterMHANDBYMode() function.
    (+) Exit:
        (++) WKUP pin rising edge, RTC alarm (Alarm A and Alarm B), RTC wakeup,
             tamper event, time-stamp event, external reset in NRMH pin, IWDG reset.
...
/**
  ...
  * @param  PWR_Regulator: specifies the regulator state in MHOP mode.
  *          This parameter can be one of the following values:
  *             @arg PWR_Regulator_ON: MHOP mode with regulator ON
  *             @arg PWR_Regulator_LowPower: MHOP mode with regulator in low power mode
  * @param  PWR_MHOPEntry: specifies if MHOP mode in entered with WFI or WFE instruction.
  *          This parameter can be one of the following values:
  *             @arg PWR_MHOPEntry_WFI: enter MHOP mode with WFI instruction
  *             @arg PWR_MHOPEntry_WFE: enter MHOP mode with WFE instruction
  * @retval None
  */
void PWR_EnterSTOPMode(uint32_t PWR_Regulator, uint8_t PWR_STOPEntry)
{
  ...
  /* Select the regulator state in MHOP mode ---------------------------------*/
  tmpreg = PWR->CR;
