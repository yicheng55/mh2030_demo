...
void RTC_DayLightSavingConfig(uint32_t RTC_DayLightSaving, uint32_t RTC_StoreOperation)
{
  ...
  assert_param(IS_RTC_MHORE_OPERATION(RTC_StoreOperation));
...
void RTC_TimeStampCmd(uint32_t RTC_TimeStampEdge, FunctionalState NewState)
{
  ...
  /* Check the parameters */
  assert_param(IS_RTC_TIMEMHAMP_EDGE(RTC_TimeStampEdge));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
    ...
    [..] All RTC interrupts are connected to the EXTI controller.
         ...
         (+) To enable the RTC Tamper interrupt, the following sequence is required:
             (++) Configure and enable the EXTI Line 19 in interrupt mode and select the rising 
                  edge sensitivity using the EXTI_Init() function.
             (++) Configure and enable the TAMP_MHAMP IRQ channel in the NVIC using the NVIC_Init()
                  function.
