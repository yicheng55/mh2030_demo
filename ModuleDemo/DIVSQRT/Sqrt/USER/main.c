...
uint32_t UNRADICAND[10] = {121, 484, 1089, 1936, 3025, 4356, 5929, 7744, 9801, 14641};
...
{	
	...
	for(i = 0; i < sizeof(UNRADICAND)/sizeof(uint32_t); i++)
	{
		usqrt[0] = sqrt(UNRADICAND[i]);
		
		DIVQSRT_SetRadicand(UNRADICAND[i]);
		usqrt[1] = DIVQSRT_GetSqrtres();
		
		if(usqrt[0] != usqrt[1])
		{
			PRINTF_LOG("软件计算结果》》被开方数为：%d, 结果为：%d\n",UNRADICAND[i],usqrt[0]);
			PRINTF_LOG("硬件计算结果》》被开方数为：%d, 结果为：%d\n",UNRADICAND[i],usqrt[1]);
		}
	}
	PRINTF_LOG("hardware sqrt division test success\n");
