...
#define DM9058_SPI_RX_DMA        DMA1_Channel2
#define DM9058_SPI_TX_DMA        DMA1_Channel3
#define DM9058_SPI_RX_DMA_FLAG   DMA1_FLAG_TC2
#define DM9058_SPI_TX_DMA_FLAG   DMA1_FLAG_TC3
#define DM9058_SPI_DMA_TIMEOUT   1000000u
...
static uint8_t DM9058_DummyTx = 0x00u;
static uint8_t DM9058_DummyRx;
...
{
    DM9058_DBG_PRINT("[DM9058 DMA DBG] GPIOA MODER=0x%08lX IDR=0x%04X ODR=0x%04X AFRH=0x%08lX\r\n",
                     GPIOA->MODER, GPIOA->IDR, GPIOA->ODR, GPIOA->AFR[1]);
    DM9058_DBG_PRINT("[DM9058 DMA DBG] GPIOB MODER=0x%08lX IDR=0x%04X ODR=0x%04X AFRL=0x%08lX\r\n",
                     GPIOB->MODER, GPIOB->IDR, GPIOB->ODR, GPIOB->AFR[0]);
    DM9058_DBG_PRINT("[DM9058 DMA DBG] GPIOF MODER=0x%08lX IDR=0x%04X ODR=0x%04X\r\n",
                     GPIOF->MODER, GPIOF->IDR, GPIOF->ODR);
    DM9058_DBG_PRINT("[DM9058 DMA DBG] Pins CS(PA15)=%u SCK(PB3)=%u MOSI(PB5)=%u MISO(PB4)=%u RST(PF7)=%u\r\n",
...
static void DM9058_DebugPrintSpiState(void)
{
    ...
    br = ((uint32_t)DM9058_SPI->CR1 >> 3u) & 0x7u;
    spi_clk = pclk >> (br + 1u);

    DM9058_DBG_PRINT("[DM9058 DMA DBG] SPI1 CR1=0x%04X CR2=0x%04X SR=0x%04X I2SCFGR=0x%04X\r\n",
                     DM9058_SPI->CR1, DM9058_SPI->CR2, DM9058_SPI->SR, DM9058_SPI->I2SCFGR);
    DM9058_DBG_PRINT("[DM9058 DMA DBG] SPI1 clock: PCLK=%lu Hz, BR=%lu (div=%lu), SPI_CLK=%lu Hz (%lu kHz)\r\n",
                     pclk, br, 1ul << (br + 1u), spi_clk, spi_clk / 1000ul);
}
#endif

static uint8_t DM9058_WaitSpiIdle(void)
{
    uint32_t timeout = DM9058_SPI_DMA_TIMEOUT;

    while (SPI_I2S_GetFlagStatus(DM9058_SPI, SPI_I2S_FLAG_BSY) == SET) {
        if (--timeout == 0u) {
#if DM9058_SPI_DEBUG
            DM9058_DBG_PRINT("[DM9058 DMA DBG] SPI busy timeout, SR=0x%04X\r\n", DM9058_SPI->SR);
#endif
            return 0u;
        }
    }
...
static void DM9058_DmaConfig(uint8_t *rx, const uint8_t *tx, uint16_t len)
{
    DMA_InitTypeDef dma;

    DMA_Cmd(DM9058_SPI_RX_DMA, DISABLE);
    DMA_Cmd(DM9058_SPI_TX_DMA, DISABLE);
    DMA_ClearFlag(DM9058_SPI_RX_DMA_FLAG | DM9058_SPI_TX_DMA_FLAG);

    DMA_DeInit(DM9058_SPI_RX_DMA);
    DMA_StructInit(&dma);
    dma.DMA_PeripheralBaseAddr = (uint32_t)&DM9058_SPI->DR;
    dma.DMA_MemoryBaseAddr = (uint32_t)rx;
    dma.DMA_DIR = DMA_DIR_PeripheralSRC;
    dma.DMA_BufferSize = len;
    dma.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
    dma.DMA_MemoryInc = (rx == &DM9058_DummyRx) ? DMA_MemoryInc_Disable : DMA_MemoryInc_Enable;
    dma.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
    dma.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
    dma.DMA_Mode = DMA_Mode_Normal;
    dma.DMA_Priority = DMA_Priority_VeryHigh;
    dma.DMA_M2M = DMA_M2M_Disable;
    DMA_Init(DM9058_SPI_RX_DMA, &dma);

    DMA_DeInit(DM9058_SPI_TX_DMA);
    DMA_StructInit(&dma);
    dma.DMA_PeripheralBaseAddr = (uint32_t)&DM9058_SPI->DR;
    ...
    dma.DMA_BufferSize = len;
    dma.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
    dma.DMA_MemoryInc = (tx == &DM9058_DummyTx) ? DMA_MemoryInc_Disable : DMA_MemoryInc_Enable;
    dma.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
    dma.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
    dma.DMA_Mode = DMA_Mode_Normal;
    dma.DMA_Priority = DMA_Priority_VeryHigh;
    dma.DMA_M2M = DMA_M2M_Disable;
    DMA_Init(DM9058_SPI_TX_DMA, &dma);
}

static uint8_t DM9058_DmaTransfer(uint8_t *rx, const uint8_t *tx, uint16_t len)
{
    uint32_t timeout;
    ...
    if (!DM9058_WaitSpiIdle()) {
        return 0u;
    }

    DM9058_DmaConfig(rx, tx, len);
    DMA_Cmd(DM9058_SPI_RX_DMA, ENABLE);
    DMA_Cmd(DM9058_SPI_TX_DMA, ENABLE);

    timeout = DM9058_SPI_DMA_TIMEOUT * (uint32_t)len;
    while (DMA_GetFlagStatus(DM9058_SPI_RX_DMA_FLAG) == RESET) {
        if (--timeout == 0u) {
            DMA_Cmd(DM9058_SPI_TX_DMA, DISABLE);
            DMA_Cmd(DM9058_SPI_RX_DMA, DISABLE);
#if DM9058_SPI_DEBUG
            DM9058_DBG_PRINT("[DM9058 DMA DBG] DMA RX timeout, len=%u SR=0x%04X\r\n", len, DM9058_SPI->SR);
#endif
            return 0u;
        }
    }
    ...
    timeout = DM9058_SPI_DMA_TIMEOUT * (uint32_t)len;
    while (DMA_GetFlagStatus(DM9058_SPI_TX_DMA_FLAG) == RESET) {
        if (--timeout == 0u) {
            DMA_Cmd(DM9058_SPI_TX_DMA, DISABLE);
            DMA_Cmd(DM9058_SPI_RX_DMA, DISABLE);
#if DM9058_SPI_DEBUG
            DM9058_DBG_PRINT("[DM9058 DMA DBG] DMA TX timeout, len=%u SR=0x%04X\r\n", len, DM9058_SPI->SR);
#endif
            return 0u;
        }
    }
...
void MH2030A_SPI1_Init(void)
{
    ...
    SPI_I2S_DMACmd(DM9058_SPI, SPI_I2S_DMAReq_Rx | SPI_I2S_DMAReq_Tx, ENABLE);
    SPI_Cmd(DM9058_SPI, ENABLE);

#if DM9058_SPI_DEBUG
    DM9058_DBG_PRINT("[DM9058 DMA DBG] SPI1 DMA init done\r\n");
...
uint8_t MH2030A_SPI1_Transfer(uint8_t tx)
{
    ...
    timeout = DM9058_SPI_DMA_TIMEOUT;
    while (SPI_I2S_GetFlagStatus(DM9058_SPI, SPI_I2S_FLAG_TXE) == RESET) {
        if (--timeout == 0u) {
#if DM9058_SPI_DEBUG
            DM9058_DBG_PRINT("[DM9058 DMA DBG] TXE timeout, SR=0x%04X\r\n", DM9058_SPI->SR);
#endif
            return 0x00u;
        }
    }
    ...
    while (SPI_I2S_GetFlagStatus(DM9058_SPI, SPI_I2S_FLAG_RXNE) == RESET) {
        if (--timeout == 0u) {
#if DM9058_SPI_DEBUG
            DM9058_DBG_PRINT("[DM9058 DMA DBG] RXNE timeout, tx=0x%02X SR=0x%04X CR1=0x%04X CR2=0x%04X\r\n",
...
{
#if DM9058_SPI_DEBUG
    DM9058_DBG_PRINT("[DM9058 DMA DBG] Hardware reset start\r\n");
#endif
    ...
    Delay_Ms(10);
#if DM9058_SPI_DEBUG
    DM9058_DBG_PRINT("[DM9058 DMA DBG] Hardware reset done, RST(PF7)=%u\r\n",
...
void DM9058_DebugDump(const char *tag)
{
    ...
    DM9058_DBG_PRINT("\r\n[DM9058 DMA DBG] dump: %s\r\n", tag ? tag : "");
    DM9058_DebugPrintPinState();
    DM9058_DebugPrintSpiState();

    DM9058_CS_Low();
    raw1 = MH2030A_SPI1_Transfer(DM9058_CHIPR | DM9058_OP_REG_R);
    raw2 = MH2030A_SPI1_Transfer(0x00u);
    DM9058_CS_High();
    DM9058_DBG_PRINT("[DM9058 DMA DBG] raw CHIPR read: cmd=0x%02X data=0x%02X\r\n", raw1, raw2);
    ...
    DM9058_DBG_PRINT("[DM9058 DMA DBG] regs NCR=0x%02X NSR=0x%02X VIDL=0x%02X VIDH=0x%02X PIDL=0x%02X PIDH=0x%02X CHIPR=0x%02X ISR=0x%02X IMR=0x%02X\r\n",
...
void DM9058_ReadRegBuf(uint8_t reg, uint8_t *buf, uint16_t len)
{
    DM9058_CS_Low();
    MH2030A_SPI1_Transfer((uint8_t)(reg | DM9058_OP_REG_R));
    (void)DM9058_DmaTransfer(buf, &DM9058_DummyTx, len);
    DM9058_CS_High();
}
...
void DM9058_WriteRegBuf(uint8_t reg, const uint8_t *buf, uint16_t len)
{
    DM9058_CS_Low();
    MH2030A_SPI1_Transfer((uint8_t)(reg | DM9058_OP_REG_W));
    (void)DM9058_DmaTransfer(&DM9058_DummyRx, buf, len);
    DM9058_CS_High();
}
