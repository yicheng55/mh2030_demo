...
int main(void)
{
    ...
    printf("[DM9051] AT32F403A polling smoke start\r\n");

    status = dm9051_at32f403a_smoke_open(dm9051_smoke_mac);
    dev = dm9051_at32f403a_smoke_device();

    printf("[DM9051] open status=%d found=%d VID=0x%04X PID=0x%04X CHIPR=0x%02X\r\n",
           status,
    ...
    while (1) {
        ...
        rx_len = dm9051_at32f403a_smoke_receive(dm9051_smoke_rx_buf,
                                                sizeof(dm9051_smoke_rx_buf));
        ++loops;

        if (rx_len != 0u) {
            printf("[DM9051] rx len=%u first=%02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\r\n",
                   rx_len,
            ...
            printf("[DM9051] rx len=0\r\n");
        }

        if ((loops % 5u) == 0u) {
            status = dm9051_at32f403a_smoke_send(dm9051_smoke_tx_buf,
                                                 sizeof(dm9051_smoke_tx_buf));
            printf("[DM9051] tx len=%u status=%d\r\n",
