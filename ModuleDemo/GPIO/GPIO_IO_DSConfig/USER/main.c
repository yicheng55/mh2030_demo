...
{	
	...
	PRINTF_LOG("GPIO_IO_DSConfig.\n");
	
	GPIO_Configuration();

	GPIO_DsConfig(GPIO_GROUP_TEST,GPIO_PIN_TEST,GPIO_Ds_Low);

	while(1)
	{
		GPIO_SetBits(GPIO_GROUP_TEST, GPIO_PIN_TEST);
		Delay_Ms(200);
		GPIO_ResetBits(GPIO_GROUP_TEST, GPIO_PIN_TEST);
		Delay_Ms(200);
	}
}
