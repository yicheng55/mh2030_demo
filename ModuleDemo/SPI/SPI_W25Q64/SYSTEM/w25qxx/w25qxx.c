...
{					 	
	while (SPI_I2S_GetFlagStatus(SPI_TEST, SPI_I2S_FLAG_TXE) == RESET); //检查指定的SPI标志位设置与否:发送缓存空标志位	  
	SPI_SendData8(SPI_TEST, TxData); //通过外设SPIx发送一个数据

	while(SPI_GetReceptionFIFOStatus(SPI_TEST) == SPI_ReceptionFIFOStatus_Empty);//检查接收FIFO是否为空
	return SPI_ReceiveData8(SPI_TEST); //返回通过SPIx最近接收的数据					    
}
...
void SPI_SetSpeed(u8 SPI_BaudRatePrescaler)
{
	...
	SPI_TEST->CR1&=0XFFC7;
	SPI_TEST->CR1|=SPI_BaudRatePrescaler;
	SPI_Cmd(SPI_TEST,ENABLE); 
...
{	
	...
	SPI_InitTest();		   	//初始化SPI
	SPI_SetSpeed(SPI_BaudRatePrescaler_128);
