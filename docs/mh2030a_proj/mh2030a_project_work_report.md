...
此圖顯示 Keil target 下拉清單，確認目前可切換 5 個 target，涵蓋基礎 SPI probe、SPI DMA、uIP、uIP DMA、uIP interrupt。此總覽畫面可先建立各 target 的功能分層，後續 2.2 到 2.6 再逐一對照各 target 主畫面、專案群組與 build output。
...
### 2.5 `MH2030A_DM9051_uIP_dma` target
...
### 2.6 `MH2030A_DM9051_uIP_int` target
...
以下依據 `DM9051A.uvprojx` 內實際 XML 設定整理。TargetName 不同時，Keil 主要透過 `OutputName`、`OutputDirectory`、C compiler `Define`、`IncludePath`、warning/C99 選項，以及各 source file 的 `IncludeInBuild` 來切換功能。

| TargetName | OutputName | OutputDir | Define | 實際編譯分流 |
| --- | --- | --- | --- | --- |
| `DM9051A` | `DM9051A` | `..\OBJ\` | `USE_STDPERIPH_DRIVER` | 基礎 SPI probe；編譯 `main.c` 與 `mh2030a_spi1.c`，排除 `main_uip_mh2030a.c`、`mh2030a_spi1_dma.c`，且 uIP/DM9051 beta/web app 群組不納入 build。 |
| `DM9051A_SPI_DMA` | `DM9051A_DMA` | `..\OBJ\` | `USE_STDPERIPH_DRIVER` | 基礎 SPI DMA probe；巨集與 include path 和 `DM9051A` 相同，真正差異是排除 `mh2030a_spi1.c` 並編譯 `mh2030a_spi1_dma.c`，同時排除 `MH2030A_PORT/mh2030a_dm9051_spi.c`。 |
| `MH2030A_DM9051_uIP` | `MH2030A_DM9051_uIP` | `..\OBJ_UIP\` | `USE_STDPERIPH_DRIVER`,<br>`MH2030A_UIP_PORT` | uIP polling/一般版本；排除 `main.c`，改由 `main_uip_mh2030a.c` 作為主流程；排除 `mh2030a_dm9051_spi_dma.c`，使用 `mh2030a_dm9051_spi.c`。 |
| `MH2030A_DM9051_uIP_dma` | `MH2030A_DM9051_uIP_dma` | `..\OBJ_UIP\` | `USE_STDPERIPH_DRIVER`,<br>`MH2030A_UIP_PORT`,<br>`MH2030A_DM9051_SPI_DMA` | uIP DMA 版本；排除 `main.c` 與 `mh2030a_dm9051_spi.c`，改編譯 `mh2030a_dm9051_spi_dma.c`；此 target 的 C warning level 為 `2`。 |
| `MH2030A_DM9051_uIP_int` | `MH2030A_DM9051_uIP_int` | `..\OBJ_UIP_INT\` | `USE_STDPERIPH_DRIVER`,<br>`MH2030A_UIP_PORT`,<br>`DMPLUG_INT` | uIP interrupt 版本；排除 `main.c` 與 `mh2030a_dm9051_spi_dma.c`，使用 polling SPI source 加上 `DMPLUG_INT` 啟用 PF6/EXTI6 interrupt path；此 target 的 C99 選項為 `1`。 |

共同設定：

- Device：`ARMCM0`
- Vendor：`ARM`
- Toolchain：`ARM-ADS` / ARMCC5，`V5.06 update 7 (build 960)`
- CPU：`Cortex-M0`
...
### 3.1 TargetName 差異對應的參數分析

`DM9051A` 與 `DM9051A_SPI_DMA` 是基本 probe 類 target。兩者共用 `..\OBJ\`、共用 `USE_STDPERIPH_DRIVER`，也共用相同 include path，因此 Keil 的 C compiler 巨集看不出 SPI/DMA 差異。差異落在 file option：`DM9051A` 編譯 `BSP/mh2030a_spi1.c` 並排除 `BSP/mh2030a_spi1_dma.c`；`DM9051A_SPI_DMA` 則反過來，編譯 DMA source 並排除一般 SPI source。這代表 `TargetName` 雖然叫 `SPI_DMA`，但不是靠 `Define` 啟用 DMA，而是靠「納入哪個 source file」切換。

`MH2030A_DM9051_uIP`、`MH2030A_DM9051_uIP_dma`、`MH2030A_DM9051_uIP_int` 是 uIP 類 target。三者都定義 `MH2030A_UIP_PORT`，因此 `main.c` 被排除，專案改用 `main_uip_mh2030a.c` 執行 board/network/web 初始化。三者也都加入 uIP middleware、application、webserver、DM9051 beta driver、`..\port\mh2030a`、`..\port\uip` 等 include path。

uIP 一般版與 uIP DMA 版的主要差異有兩層：第一層是 `MH2030A_DM9051_uIP_dma` 額外定義 `MH2030A_DM9051_SPI_DMA`；第二層是 file option 排除 `mh2030a_dm9051_spi.c`，改編譯 `mh2030a_dm9051_spi_dma.c`。因此 DMA 版本同時具備巨集分流與 source file 分流，設定比基礎 `DM9051A_SPI_DMA` 更明確。

uIP interrupt 版的差異集中在 `DMPLUG_INT`。`MH2030A_DM9051_uIP_int` 不切到 DMA source，而是沿用 `mh2030a_dm9051_spi.c`，並透過 `DMPLUG_INT` 啟用 `mh2030a_dm9051_int.c` 中 PF6/EXTI6 的中斷初始化與 ISR 通知流程。它使用獨立輸出目錄 `..\OBJ_UIP_INT\`，可避免與 polling/DMA uIP target 的中間檔混在一起。

編譯器細部選項也有 target 差異：五個 target 的 optimization 都是 `1`，但 `MH2030A_DM9051_uIP_dma` 的 warning level 是 `2`，其他 target 是 `0`；`MH2030A_DM9051_uIP_int` 的 C99 選項是 `1`，其他 target 是 `0`。這兩項目前不是功能名稱直接可見的差異，後續若追求 target 一致性，可再評估是否要統一。
...
所有 target 目前都列出相同的主要群組，但是否參與 build 會依 target 的 `IncludeInBuild` 切換：
...
實際編譯入口與功能由巨集和 file option 共同控制：

- `DM9051A` 排除 `mh20xx_it.c`、`main_uip_mh2030a.c`、`mh2030a_spi1_dma.c`，並排除 uIP/DM9051 beta/web app 相關群組。
- `DM9051A_SPI_DMA` 排除 `mh20xx_it.c`、`main_uip_mh2030a.c`、`mh2030a_spi1.c`、`mh2030a_dm9051_spi.c`，並排除 uIP/DM9051 beta/web app 相關群組。
- `MH2030A_DM9051_uIP` 排除 `main.c` 與 `mh2030a_dm9051_spi_dma.c`。
- `MH2030A_DM9051_uIP_dma` 排除 `main.c` 與 `mh2030a_dm9051_spi.c`。
- `MH2030A_DM9051_uIP_int` 排除 `main.c` 與 `mh2030a_dm9051_spi_dma.c`。
- `main.c` 外層使用 `#ifndef MH2030A_UIP_PORT`，搭配 file option，只在非 uIP target 編譯主要 probe 流程。
