...
struct netif *ethernetif_register(void)
{
    ...
    memset(&eth_netif, 0, sizeof(eth_netif));

    eth_netif.hwaddr[0] = 0x00;
    eth_netif.hwaddr[1] = 0x60;
    eth_netif.hwaddr[2] = 0x6E;
    eth_netif.hwaddr[3] = 0x11;
    eth_netif.hwaddr[4] = 0x22;
    eth_netif.hwaddr[5] = 0x33;

    if (netif_add(&eth_netif, &ipaddr, &netmask, &gateway,
...
{
    if (macadd != NULL) {
        memcpy(eth_netif.hwaddr, macadd, ETH_HWADDR_LEN);
    }
}

int ethernetif_poll(void)
{
    if (ethernetif_input(&eth_netif) != ERR_OK) {
        return -1;
    }
    return 0;
}
